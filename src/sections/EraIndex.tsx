...
export function EraIndex({ language, onPlayAlbum }: { language: Language; onPlayAlbum: (albumId: string) => void }) {
  ...
  useEffect(() => { setSelected((index) => Math.min(index, Math.max(0, albums.length - 1))); }, [albums.length]);
  ...
  const era = albums[selected] ?? albums[0];
  ...
  return (
    <section id="archive" className="era-index" style={{ '--era-accent': era.colorAccent } as CSSProperties}>
      ...
      <div className="index-layout">
        <div ref={selectorRef} className="era-selector" role="group" aria-label={zh ? '选择时代' : 'Choose an era'}>{albums.map((item, i) => <button key={item.id} aria-pressed={selected === i} onClick={() => setSelected(i)} className={selected === i ? 'selected' : ''}><span>{item.number}</span><span>{item.name.en}</span>{item.year && <small>{item.year}</small>}<span aria-hidden="true">↗</span></button>)}</div>
